...
void * run_co_calvinlockthread(void * id);
void * run_co_calvinseqthread(void * id);
...
#if USE_COROUTINE
	pthread_create(&p_thds[id++], &attr, run_co_calvinlockthread, (void *)&calvin_lock_thds[0]);
#else
...
#if USE_COROUTINE
	pthread_create(&p_thds[id++], &attr, run_co_calvinseqthread, (void *)&calvin_seq_thds[0]);
#else
...
void * run_co_calvinlockthread(void * id) {
	CalvinLockThread * thd = (CalvinLockThread *) id;
	thd->create_routines(COROUTINE_CNT);
	thd->start_routine();
	return NULL;
}
void * run_co_calvinseqthread(void * id) {
	CalvinSequencerThread * thd = (CalvinSequencerThread *) id;
	thd->create_routines(COROUTINE_CNT);
	thd->start_routine();
	return NULL;
}
