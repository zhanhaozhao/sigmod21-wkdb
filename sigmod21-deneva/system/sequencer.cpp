...
#if USE_COROUTINE
void Sequencer::register_thread(Thread * h_thd, uint64_t cor_id) {
	this->h_thd = h_thd;
	this->_cor_id = cor_id;
}

void Sequencer::send_next_batch(yield_func_t &yield, uint64_t thd_id, uint64_t cor_id) {
	uint64_t prof_stat = get_sys_clock();
	qlite_ll * en = wl_tail;
	bool empty = true;
	if(en && en->epoch == simulation->get_seq_epoch()) {
	...
	for(uint64_t j = 0; j < g_node_cnt; j++) {
		if(j == g_node_id) {
			...
			while(fill_queue[j].pop(msg)) {
				assert(ISSERVERN(msg->return_node_id));
				temp_queue->add_txnentry(yield, h_thd, msg, cor_id);
			}
			temp_queue->add_txnentry(yield, h_thd, Message::create_message(RDONE), cor_id);
		}	
		else {
			assert(j != g_node_id);
			temp_queue = (sched_queue *)mem_allocator.alloc(sizeof(sched_queue));
			calvin_man.read_remote_queue(yield, cor_id, h_thd, temp_queue,j);
			
			//可以使用door-bell优化
			while(fill_queue[j].pop(msg)) {
				assert(ISSERVERN(msg->return_node_id));
				temp_queue->write_entry(yield, cor_id, h_thd, msg, j);
				// temp_queue->write_rear(thd_id, j);
			}
			msg = Message::create_message(RDONE);
			temp_queue->write_entry(yield, cor_id, h_thd, msg, j);
			temp_queue->write_rear(yield, cor_id, h_thd, j);
			mem_allocator.free(temp_queue,sizeof(sched_queue));
		}
		if(!empty) {
			DEBUG("Seq RDONE %ld\n",simulation->get_seq_epoch())
