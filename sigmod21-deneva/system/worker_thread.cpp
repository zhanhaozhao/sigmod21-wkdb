...
#if USE_COROUTINE
  cor_txn_man[cor_id]->release_locks(yield, RCOK, cor_id);
  cor_txn_man[cor_id]->commit_stats();
  DEBUG("(%ld,%ld) calvin ack to %ld\n", cor_txn_man[cor_id]->get_txn_id(), cor_txn_man[cor_id]->get_batch_id(),
        cor_txn_man[cor_id]->return_id);
  if(cor_txn_man[cor_id]->return_id == g_node_id) {
    work_queue.sequencer_enqueue(_thd_id,Message::create_message(cor_txn_man[cor_id],CALVIN_ACK));
  } else {
#if USE_RDMA == CHANGE_MSG_QUEUE
    tport_man.rdma_thd_send_msg(get_thd_id(), cor_txn_man[cor_id]->return_id, Message::create_message(cor_txn_man[cor_id], CALVIN_ACK));
#else
    msg_queue.enqueue(get_thd_id(), Message::create_message(cor_txn_man[cor_id], CALVIN_ACK),
                      cor_txn_man[cor_id]->return_id);
#endif
  }
  co_release_txn_man(cor_id);
...
RC WorkerThread::process_calvin_rtxn(yield_func_t &yield, Message * msg, uint64_t cor_id) {
  ...
  DEBUG("START %ld %f %lu\n", cor_txn_man[cor_id]->get_txn_id(),
        simulation->seconds_from_start(get_sys_clock()), cor_txn_man[cor_id]->txn_stats.starttime);
  assert(ISSERVERN(cor_txn_man[cor_id]->return_id));
  cor_txn_man[cor_id]->txn_stats.local_wait_time += get_sys_clock() - cor_txn_man[cor_id]->txn_stats.wait_starttime;
  // Execute
  RC rc = cor_txn_man[cor_id]->run_calvin_txn(yield, cor_id);
  // if((txn_man->phase==6 && rc == RCOK) || txn_man->active_cnt == 0 || txn_man->participant_cnt ==
  // 1) {
  if(rc == RCOK && cor_txn_man[cor_id]->calvin_exec_phase_done()) {
    calvin_wrapup(yield, cor_id);
  }
  return RCOK;
#else
