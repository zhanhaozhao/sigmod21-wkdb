...
#if USE_COROUTINE
void CalvinLockThread::create_routines(int coroutines) {
    _routines = new coroutine_func_t[1 + coroutines];

    _routines[0] = coroutine_func_t(bind(&CalvinLockThread::master_routine, this, _1, 0));
    for(uint64_t i = 1; i <= coroutines; ++i) {
    	_routines[i] = coroutine_func_t(bind(&CalvinLockThread::co_run, this, _1, i));
    }
    printf("Thread %ld Init coroutine succ, num:%d\n",_thd_id, coroutines);
}

void CalvinLockThread::master_routine(yield_func_t &yield, int cor_id) {
	tsetup();
	printf("Running CalvinLockThread %ld master routine:%ld\n",_thd_id, cor_id);
	while(!simulation->is_done()) {
...
RC CalvinLockThread::co_run(yield_func_t &yield, uint64_t cor_id){
	// tsetup();
	printf("Running CalvinLockThread %ld:%ld\n",_thd_id, cor_id);
	RC rc = RCOK;
	TxnManager * txn_man;
	uint64_t prof_starttime = get_sys_clock();
	uint64_t idle_starttime = 0;
	...
	while(!simulation->is_done()) {
		...
		txn_man->register_thread(this);
#else
		txn_man->register_thread(this, cor_id);
#endif
		assert(ISSERVERN(txn_man->return_id));
...
void CalvinSequencerThread::create_routines(int coroutines) {
    _routines = new coroutine_func_t[1 + coroutines];

    _routines[0] = coroutine_func_t(bind(&CalvinSequencerThread::master_routine, this, _1, 0));
    for(uint64_t i = 1; i <= coroutines; ++i) {
    	_routines[i] = coroutine_func_t(bind(&CalvinSequencerThread::co_run, this, _1, i));
    }
    printf("Thread %ld Init coroutine succ, num:%d\n",_thd_id, coroutines);
}

void CalvinSequencerThread::master_routine(yield_func_t &yield, int cor_id) {
	tsetup();
	printf("Running CalvinSequencerThread %ld master routine:%ld\n",_thd_id, cor_id);
	while(!simulation->is_done()) {
        last_yield_time = get_sys_clock();
        yield(_routines[1]);
        uint64_t yield_endtime = get_sys_clock();
...
RC CalvinSequencerThread::co_run(yield_func_t &yield, uint64_t cor_id) {
	// tsetup();
	printf("Running CalvinSequencerThread %ld:%ld\n",_thd_id, cor_id);
	...
	while(!simulation->is_done()) {
		...
		if(is_batch_ready()) {
			simulation->advance_seq_epoch();
			//last_batchtime = get_wall_clock();
			seq_man.send_next_batch(yield, _thd_id, cor_id);
		}
