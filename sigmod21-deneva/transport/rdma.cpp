...
#if USE_COROUTINE
char* Rdma::get_queue_client_memory(uint64_t cor_id) {
	char * temp =  (char *)(client_rdma_rm->raw_ptr) + g_msg_size * cor_id;
  	return temp;
}

char* Rdma::get_rear_client_memory(uint64_t cor_id) {
	char * temp =  (char *)(client_rdma_rm->raw_ptr) + g_msg_size * COROUTINE_CNT;
	temp += sizeof(uint64_t) * cor_id;
  	return temp;
}
#endif
